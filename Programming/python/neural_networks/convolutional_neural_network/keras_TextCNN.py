...
class TextCNN_Keras():
    ...
    def train(self):
        ...
        batches_dev = data_helpers.batch_iter(
            list(zip(self.x_dev, self.y_dev)), self.FLAGS.batch_size, self.FLAGS.num_epochs)
        counter = 1
        for batch in batches:
            ...
            if counter % 500 == 0:
                self.cnn.save("keras_model/one_layer_latest_model.h5")
                self.predict(counter, batches_dev)
            counter += 1

    def predict(self, counter, batches_dev):
        acc_mse = []
